...
module.exports = {
  ...
  save: async (req, res) => {
    let patient_body = {
      ...
      smoking: req.body.tabaquismo,
      date_smoking: req.body.fecha_tabaquismo,
      date_end_smoking: req.body.fecha_end_tabaquismo,
      action_smoking: req.body.accion_tabaquismo,
      coments_smoking: req.body.comentario_tabaquismo,
      other_disease: req.body.otras_enfermedades,
  ...
  updatePatients: async (req, res) => {
    ...
    let errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.render(path.resolve(__dirname, '..', 'views', 'patients', 'patientsEdit'), {
        paciente,
        errors: errors.errors, old: req.body
      });
    }
  ...
  tabaquismo: async (req, res) => {
    let paciente = await patients.findAll({ where: { smoking: 'si' } })
    let patient = paciente.sort(function (a, b) {
      if (a.last_name.toLowerCase() < b.last_name.toLowerCase()) {
        return -1
      } else if (a.first_name.toLowerCase() > b.first_name.toLowerCase()) {
  ...
  historiaclinica: async (req, res) => {
    ...
    const historiaClinica = await medicalhistories.findAll({
      where: {
        patient_id: req.params.id, visitamedica: {
          [Op.ne]: '%null%'
        }
      }
    })
    ...
    const exam = await exams.findAll({
      where: {
        patient_id: req.params.id, altura: {
          [Op.ne]: '%null%'
        }
      }
    })
  ...
  addevolution: async (req, res) => {
    ...
    let newexams = {
      ...
      frec_resp: req.body.frec_resp,
      temp_axilar: req.body.temp_axilar,
      cardiologia: req.body.cardiologia,
      doctor: req.body.doctor,
    }
  ...
  addimage: async (req, res) => {
    ...
    let errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.render(path.resolve(__dirname, '..', 'views', 'patients', 'patientHistory'), {
        paciente, historiaClinica, exam, imagenes,
        errors: errors.errors, old: req.body
      });
    }
